#!/usr/bin/env node
/**
 * Update SPX (S&P 500) monthly return data from Stooq.
 * Appends any new months not already in the JSON file.
 * Format: [{ "date": "YYYY-MM", "return": -0.071464 }, ...]
...
const DATA_FILE = join(__dirname, '..', 'src', 'data', 'spx.json');
...
function toYYYYMM(dateStr) {
  // dateStr could be "2025-01-31" or "2025-01"
  return dateStr.slice(0, 7);
}

async function main() {
  console.log('Fetching SPX monthly data from Stooq...');
  const csv = await fetchStooqCSV('^spx', 'm');
  ...
  if (newEntries.length === 0) {
    console.log(`SPX: already up to date (last: ${lastExisting})`);
    return;
  }

  const updated = [...existing, ...newEntries];
  writeFileSync(DATA_FILE, JSON.stringify(updated, null, 2) + '\n');
  console.log(`SPX: added ${newEntries.length} new month(s). Last: ${newEntries[newEntries.length - 1].date}`);
}

main().catch(err => {
  console.error('SPX update failed:', err.message);
  process.exit(1);
});
